 ...
 * Flex-Wrap Centering for Mobile
 */
(function() {
    ...
    function injectCSS() {
        ...
        style.innerHTML = `
            :root {
                --brand-primary: #2a206a; --brand-hover: #f26622; --modal-bg: #f2f2f2;
            ...
            .wizard-header {
                background: url('https://cos-cdn-new.s3.us-east-1.amazonaws.com/preference-assets/images/header-gradient-bkg.jpg') center/cover no-repeat;
            ...
            .wizard-close { cursor: pointer; background: none; border: none; color: white; font-size: 1.2rem; padding: 5px; opacity: 0.8; transition: 0.2s; position: absolute; right: 20px; top: 50%; transform: translateY(-50%); }
            ...
            .cat-chip svg { width: 32px; height: 32px; color: var(--brand-primary); }
            .cat-chip.selected svg { color: var(--brand-hover); }

            .chip-group { display: flex; flex-wrap: wrap; gap: 10px; justify-content: center; }
            .chip { padding: 10px 18px; background: white; border: 1.5px solid #e2e8f0; border-radius: 30px; cursor: pointer; font-family: inherit; font-weight: 600; font-size: 0.9rem; }
            ...
            .btn-next { justify-self: end; background: var(--brand-primary); color: white; border: none; padding: 12px 24px; border-radius: 10px; font-weight: 700; cursor: pointer; text-transform: uppercase; letter-spacing: 0.5px; min-width: 120px; width: auto !important; }
            .btn-back { justify-self: start; background: none; border: none; color: var(--text-muted); font-weight: 700; cursor: pointer; text-transform: uppercase; font-size: 0.85rem; min-width: 120px; width: auto !important; }
            ...
            @media (max-width: 600px) {
                ...
                /* Switch from Grid to Flex Wrap for Categories and Artists */
                ...
                /* calc(50% - 6px) accounts for exactly half the container minus half the 12px gap */
                .cat-chip, .artist-card {
                    width: calc(50% - 6px);
                    flex: 0 0 auto;
                }

                /* Slightly scale down artist card to fit nicely in 2 columns */
                ...
                .btn-back, .btn-next { min-width: 0; width: 100% !important; text-align: center; }
            }
        `;
    ...
    window.openOnboardingWizard = async function({ userEmail }) {
        ...
        // Background fetches
        fetch("/wp-json/consequence/v1/onboarding-tags").then(r => r.json()).then(tags => {
        ...
        fetch("https://concerts.consequence.net/api/popular-artists").then(r => r.json()).then(data => {
            ...
            artists.slice(0, 20).forEach(a => {
                ...
                card.innerHTML = `<div class="avatar" style="background-image:url('${a.image || ONBOARDING_ARTIST_FALLBACK_IMAGE}')"></div><div class="artist-name">${a.name}</div>`;
                card.onclick = () => {
        ...
        wizard.querySelector("#wizard-main-btn").onclick = async () => {
            ...
            try {
                ...
                deleteNewUserCookie(); localStorage.setItem('wizard_completed', 'true'); wizard.remove();
            }
        };
    };
